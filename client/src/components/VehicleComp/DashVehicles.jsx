...
export default function DashVehicles() {
  ...
  const handleShowMore = async () => {
    ...
    try {
      const res = await fetch(
        `/api/vehicles/getvehicles?startIndex=${startIndex}`
      );
      const data = await res.json();
      if (res.ok) {
  ...
  const handleDeleteVehicle = async () => {
    ...
    try {
      const res = await fetch(
        `/api/vehicles/deletevehicle/${vehicleIdToDelete}/${currentUser._id}`,
        {
          method: "DELETE",
        }
      );
  ...
  return (
    <div
      className="lg:mr-32 lg:ml-10 w-full table-auto overflow-x-scroll md:mx-auto p-3 scrollbar scrollbar-track-slate-100 
      scrollbar-thumb-slate-300 dark:scrollbar-track-slate-700 dark:scrollbar-thumb-slate-500"
    >
      {/* Title and Add Button */}
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-3xl font-semibold text-gray-900 ">
          <span className="text-[#F4AC20]">VEHICLES</span> DETAILS
        </h2>
        <Link to="/dashboard?tab=createvehicle">
          <Button
            style={{ backgroundColor: "#F4AC20", borderColor: "#F4AC20" }}
          >
            Add a vehicle
          </Button>
        </Link>
